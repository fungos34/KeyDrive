...
class DiskIdentity:
    ...
    def matches(self, other: "DiskIdentity") -> bool:
        ...
        _safety_logger.debug(f"Identity comparison: '{self_id[:20]}...' vs '{other_id[:20]}...' = {match_result}")
        return match_result
...
def detect_source_disk_windows(script_path: Path) -> Optional[DiskIdentity]:
    ...
    try:
        result = subprocess.run(
            ["powershell", "-NoProfile", "-Command", ps_script], capture_output=True, text=True, timeout=30
        )
...
def detect_source_disk_unix(script_path: Path) -> Optional[DiskIdentity]:
    ...
    try:
        ...
        result = subprocess.run(["df", str(script_path)], capture_output=True, text=True, timeout=10)
        ...
        identity = DiskIdentity(unique_id=unique_id, device_path=base_device, friendly_name=base_device)
...
def get_target_disk_identity_unix(device_path: str) -> Optional[DiskIdentity]:
    ...
    try:
        ...
        return DiskIdentity(unique_id=unique_id, device_path=device_path, friendly_name=device_path)
...
class SetupSafetyPolicy:
    ...
    def validate_target(
        self, source_disk: Optional[DiskIdentity], target_disk: Optional[DiskIdentity]
        ...
        if target_disk is None or not target_disk.unique_id:
            ...
            return SafetyValidationResult.block(
                SafetyBlockReason.INVALID_DISK_ID, "Target disk identity could not be determined"
            )
        ...
        if self.setup_type == SetupType.DESTRUCTIVE_DISK:
            if source_disk is None or not source_disk.unique_id:
                ...
                return SafetyValidationResult.block(
                    ...
                    "Source disk identity could not be determined. " "Refusing to proceed with destructive operation.",
                )
            ...
            if source_disk.matches(target_disk):
                _safety_logger.critical(
                    "🛑 BLOCKED: TARGET DISK IS THE SOURCE DISK! " f"UniqueId={target_disk.unique_id}"
                )
                _safety_logger.info("=" * 70)
                return SafetyValidationResult.block(
    ...
    def validate_before_partition(
        cls, script_path: Path, target_disk_number: Optional[int] = None, target_device_path: Optional[str] = None
    ) -> SafetyValidationResult:
        """
        ...
        if system == "windows" and target_disk_number is not None:
            ...
            return SafetyValidationResult.block(
                SafetyBlockReason.INVALID_DISK_ID, "No target disk number or device path provided"
            )
...
class DiskSnapshot:
    ...
    def log(self) -> None:
        ...
        for p in self.partitions:
            ...
            _safety_logger.info(
                f"  #{p.partition_number}: {p.size_gb:.2f} GB, Type={p.partition_type}{letter_str}{hidden_str}"
            )
        ...
        if self.launcher_partition:
            _safety_logger.info(
                f"  Launcher: Partition #{self.launcher_partition.partition_number} ({self.launcher_partition.drive_letter or 'no letter'})"
            )
        else:
            _safety_logger.info("  Launcher: NOT RESOLVED")
        if self.payload_partition:
            _safety_logger.info(
                f"  Payload: Partition #{self.payload_partition.partition_number} ({self.payload_partition.drive_letter or 'no letter'})"
            )
        else:
...
def get_disk_snapshot_windows(disk_number: int, launcher_drive_letter: Optional[str] = None) -> Optional[DiskSnapshot]:
    ...
    try:
        ...
        if launcher_ref:
            candidates = [
                p for p in partitions if p.partition_number != launcher_ref.partition_number and not p.is_hidden
            ]
            if candidates:
...
def resolve_launcher_partition_windows(disk_number: int, drive_root: Optional[Path] = None) -> Optional[PartitionRef]:
    """
...
def resolve_payload_partition_windows(
    disk_number: int, launcher_partition: Optional[PartitionRef] = None
