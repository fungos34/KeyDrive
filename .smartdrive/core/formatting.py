...
try:
    ...
    class ConsoleStyle:
        ...
        def symbol(self, name):
            return f"[{name}]"
        @property
        def SUCCESS(self): return "[OK]"
        @property
        def FAILURE(self): return "[X]"
        @property
        def WARNING(self): return "[!]"
        @property
        def INFO(self): return "[i]"
...
class Formatter:
    ...
    def table(
        ...
        # Header
        if headers:
            header_cells = [str(h).ljust(col_widths[i])[:col_widths[i]] 
                          for i, h in enumerate(headers)]
        ...
        # Rows
        for row in rows:
            cells = [str(c).ljust(col_widths[i])[:col_widths[i]] 
                    for i, c in enumerate(row[:num_cols])]
...
if __name__ == "__main__":
    ...
    fmt.bullet("Second item with a much longer description that will need "
    ...
    fmt.summary_box("Setup Complete", [
        ("Drive", "D:"),
        ("Mount Point", "V:"),
