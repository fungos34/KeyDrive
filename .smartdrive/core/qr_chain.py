...
class ChunkInfo:
    ...
    def to_qr_string(self) -> str:
        ...
        """
        return f"{self.data_type}:{CHUNK_VERSION}:{self.index}/{self.total}:{self.full_hash}:{self.data}:{self.checksum}"
...
def decode_chunks_to_file(
    chunks: List[Union[str, ChunkInfo]],
    output_path: Path
) -> Tuple[bool, List[str]]:
    """
