...
class VolumeIdentifier:
    ...
    def to_config(self) -> dict:
        ...
        return {"kind": self.kind.value, "value": self.value, "resolution_method": self.resolution_method}
    ...
    def from_config(cls, data: dict) -> "VolumeIdentifier":
        ...
        if kind == VolumeIdentifierKind.DEVICE_PATH:
            raise ValueError(
                f"Invalid config: device_path is not a valid persisted identifier kind. " f"Config corruption detected."
            )
        return cls(kind=kind, value=data["value"], resolution_method=data.get("resolution_method"))
    ...
    def from_drive_letter(cls, letter: str, resolution_method: str = "explicit") -> "VolumeIdentifier":
        ...
        return cls(kind=VolumeIdentifierKind.DRIVE_LETTER, value=f"{normalized}:", resolution_method=resolution_method)
    ...
    def from_volume_guid(cls, guid_path: str, resolution_method: str = "explicit") -> "VolumeIdentifier":
        ...
        return cls(kind=VolumeIdentifierKind.VOLUME_GUID, value=guid_path, resolution_method=resolution_method)
