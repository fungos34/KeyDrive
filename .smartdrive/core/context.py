...
class RuntimeContext:
    ...
    def __post_init__(self):
        ...
        _context_logger.debug(
            f"RuntimeContext created: drive_root={self.drive_root}, " f"config_path={self.config_path}"
        )
    ...
    def from_config_path(cls, config_path: Path) -> "RuntimeContext":
        ...
        if smartdrive_dir.name != ".smartdrive":
            _context_logger.warning(f"Config parent is not .smartdrive: {smartdrive_dir.name}")

        return cls(drive_root=drive_root, smartdrive_dir=smartdrive_dir, config_path=config_path)
    ...
    def from_script_location(cls, script_path: Path) -> "RuntimeContext":
        ...
        _context_logger.debug(f"Inferred context from {script_path}: " f"drive_root={drive_root}, config={config_path}")
        ...
        if self.config_exists():
            try:
                ...
                _context_logger.info(
                    f"config.drive_id: {cfg.get('drive_id', 'N/A')[:8]}..."
                    if cfg.get("drive_id")
                    else "config.drive_id: N/A"
                )
