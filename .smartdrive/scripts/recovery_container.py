...
def check_crypto_available():
    ...
    if not CRYPTO_AVAILABLE:
        raise RecoveryContainerError("cryptography library not installed. Install with: pip install cryptography")
...
def create_container(credentials: dict, phrase: str, compress: bool = True) -> bytes:
    """
    ...
    has_creds = any(k in credentials for k in ["mount_password", "keyfile_bytes_b64", "seed_bytes_b64"])
    if not has_creds:
        raise RecoveryContainerError("Container must include at least one credential type")
...
def decrypt_container(container: bytes, phrase: str) -> dict:
    """
    ...
    if magic != CONTAINER_MAGIC:
        raise RecoveryContainerError(f"Invalid container magic: {magic!r} (expected {CONTAINER_MAGIC!r})")

    version = container[4]
    if version != CONTAINER_VERSION:
        raise RecoveryContainerError(f"Unsupported container version: {version} (expected {CONTAINER_VERSION})")
    ...
    try:
        ...
        raise RecoveryContainerError("Decryption failed - wrong phrase or corrupted container")
...
def chunk_container_for_paper(container: bytes, chunk_size: int = 1800) -> list[str]:
    """
...
def reconstruct_from_chunks(chunks: list[str]) -> bytes:
    ...
    for chunk_str in chunks:
        ...
        if total_count is None:
            ...
            if count != total_count:
                raise RecoveryContainerError(f"Inconsistent chunk count: {count} vs {total_count}")
    ...
    if len(parsed_chunks) != total_count:
        ...
        raise RecoveryContainerError(f"Missing chunks: {sorted(missing)} (have {len(parsed_chunks)}/{total_count})")
    ...
    if actual_hash != expected_hash:
        raise RecoveryContainerError(f"Container hash mismatch: {actual_hash} vs {expected_hash}")
