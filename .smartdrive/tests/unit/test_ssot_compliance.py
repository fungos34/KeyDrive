...
class TestFileNamesSSotCompliance:
    ...
    def test_no_directory_attributes_in_filenames(self):
        ...
        suspicious = [
            attr for attr in VALID_FILENAMES_ATTRS
        ...
        for py_file in get_all_python_files():
            ...
            for line_num, attr in refs:
                if attr in potential_dir_constants:
                    violations.append(
                        f"{py_file.name}:{line_num} - FileNames.{attr} "
                        f"should use Paths class instead"
                    )
...
class TestConstantsImports:
    ...
    def test_filenames_imported_when_used(self, py_file):
        ...
        if uses_filenames and not imports_filenames:
            pytest.fail(
                f"{py_file.name} uses FileNames but doesn't import it from core.constants"
            )
    ...
    def test_paths_imported_when_used(self, py_file):
        ...
        imports_paths = (
            'from core.paths import' in content and 'Paths' in content
        ) or 'import core.paths' in content
        
        if uses_paths and not imports_paths:
            pytest.fail(
                f"{py_file.name} uses Paths but doesn't import it from core.paths"
            )
