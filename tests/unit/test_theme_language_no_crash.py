...
def _read_gui() -> str:
    return _read_file(Path(Paths.SMARTDRIVE_DIR_NAME) / Paths.SCRIPTS_SUBDIR / "gui.py")


def _read_launcher() -> str:
    return _read_file(Path(Paths.SMARTDRIVE_DIR_NAME) / Paths.SCRIPTS_SUBDIR / "gui_launcher.py")
...
class TestPathsModule:
    ...
    def test_logs_dir_exists_in_paths(self):
        ...
        content = _read_file(Path(Paths.SMARTDRIVE_DIR_NAME) / "core" / "paths.py")
